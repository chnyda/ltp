...
static void setup(void)
{
	...
	SAFE_IOCTL(NULL, fd, UI_SET_EVBIT, EV_KEY, NULL);
	SAFE_IOCTL(NULL, fd, UI_SET_KEYBIT, BTN_LEFT, NULL);
	SAFE_IOCTL(NULL, fd, UI_SET_EVBIT, EV_REL, NULL);
	SAFE_IOCTL(NULL, fd, UI_SET_RELBIT, REL_X, NULL);
	SAFE_IOCTL(NULL, fd, UI_SET_RELBIT, REL_Y, NULL);
...
static int check_information(void)
{
	...
	while (nb < NB_TEST * 3 && !fail) {
		...
		if (rd == 0 || rd % sizeof(struct input_event) != 0) {
			tst_resm(TINFO, "Unexpected return value of read: %i", rd);
			fail = 1;
		}
